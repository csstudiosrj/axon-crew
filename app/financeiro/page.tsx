...
export default function FinanceiroPage() {
  ...
  useEffect(() => { if (companyId) carregarFinanceiro(); }, [companyId]);
  ...
  const carregarFinanceiro = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        eventos ( titulo, data, company_id )
      `)
      .eq("eventos.company_id", companyId!);
    ...
    if (data) {
      const formatados: Pagamento[] = data
        ...
        .map((item: any) => {
          ...
          if (statusReal === "pendente" && dataEvento && dataEvento < hoje) statusReal = "atrasado";
  ...
  const exportarCSV = () => {
    const linhas = [
      ...
      ...pagamentosFiltrados.map((p) => [
        p.freelaNome, p.eventoTitulo, formatDataBR(p.eventoData),
        p.valor.toString(), p.chavePix ?? "Não cadastrada", p.status, formatDataBR(p.dataPagamento),
      ]),
    ];
    const csvContent = linhas.map((row) => row.map((cell) => `"${cell}"`).join(";")).join("\n");
    const blob = new Blob(["\uFEFF" + csvContent], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `financeiro_arxum_crew_${hoje}.csv`;
    link.click();
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto w-full relative">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setModalPix(true)}
            ...
            {pendentesLote.length > 0 && (
              <span className="bg-green-400/20 text-green-200 text-xs px-1.5 py-0.5 rounded-full">{pendentesLote.length}</span>
            )}
          </button>
        </div>
      </div>
      ...
      {semPix.length > 0 && (
        <div className="mb-6 flex items-start gap-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3">
          ...
          <p className="text-sm text-yellow-400">
            <strong>{semPix.length} freelancer{semPix.length > 1 ? "s" : ""}</strong> com pagamento pendente sem chave PIX:{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-[#121212] border border-[#222] p-6 rounded-xl relative overflow-hidden">
          ...
          <p className="text-xs text-green-500 flex items-center gap-1"><ArrowDownRight size={13} /> Diárias quitadas</p>
        </div>
        <div className={`p-6 rounded-xl relative overflow-hidden border ${totais.atrasado > 0 ? "bg-red-500/5 border-red-500/20" : "bg-[#121212] border-[#222]"}`}>
          ...
          <p className={`text-sm font-medium mb-1 ${totais.atrasado > 0 ? "text-red-400" : "text-gray-400"}`}>Atrasados</p>
          <p className={`text-3xl font-bold mb-2 ${totais.atrasado > 0 ? "text-red-500" : "text-white"}`}>{formatBRL(totais.atrasado)}</p>
          {totais.atrasado > 0 ? (
            <p className="text-xs text-red-400 flex items-center gap-1"><ArrowUpRight size={13} /> {totais.qtdAtrasado} em atraso</p>
          ) : (
            <p className="text-xs text-green-500 flex items-center gap-1"><CheckCircle2 size={13} /> Tudo em dia!</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por freelancer ou evento..." value={busca} onChange={(e) => setBusca(e.target.value)}
              ...
              {isLoading ? (
                ...
                pagamentosFiltrados.map((pagamento) => (
                  <tr key={pagamento.id} className="hover:bg-[#161616]/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-white">{formatBRL(pagamento.valor)}</td>
                    <td className="px-6 py-4">
                      {pagamento.chavePix ? (
                        <button onClick={() => copiarPix(pagamento.chavePix!)} title="Clique para copiar"
                        ...
                        <span className="flex items-center gap-1 text-yellow-600 text-xs"><AlertCircle size={12} /> Não cadastrada</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      ...
                      {pagamento.status === "pago" && (
                        <div>
                          <span className="bg-green-500/10 text-green-500 border border-green-500/20 px-2 py-1 rounded text-xs font-medium">Pago</span>
                          {pagamento.dataPagamento && <p className="text-[10px] text-gray-600 mt-1">{formatDataBR(pagamento.dataPagamento)}</p>}
                        </div>
                      )}
                    ...
                    <td className="px-6 py-4 text-right">
                      {pagamento.status !== "pago" ? (
                        <button onClick={() => setModalConfirmarPgto(pagamento)}
                          className="bg-transparent border border-[#333] hover:border-green-500 hover:text-green-500 text-gray-400 px-3 py-1.5 rounded-md text-xs font-medium transition-all duration-200 active:scale-95">
                          Marcar Pago
                        </button>
                      ) : (
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#121212] border border-[#222] rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <div className="bg-[#0a0a0a] border border-[#333] rounded-lg p-3 mb-4 text-left">
              ...
              {modalConfirmarPgto.chavePix && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <button onClick={() => copiarPix(modalConfirmarPgto.chavePix!)}
              ...
              {!modalConfirmarPgto.chavePix && (
                <p className="text-xs text-yellow-600 mt-1 flex items-center gap-1"><AlertCircle size={11} /> Chave PIX não cadastrada</p>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleMarcarPago} className="flex-1 py-2.5 rounded-lg bg-green-600 hover:bg-green-500 text-white font-medium transition-all duration-200 active:scale-95">Sim, Confirmar</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-[#121212] border border-[#222] rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <div className="p-5 max-h-[60vh] overflow-y-auto space-y-3">
              {pendentesLote.length === 0 ? (
                ...
                <>
                  <p className="text-sm text-gray-400">Copie as chaves abaixo para realizar os pagamentos no app do seu banco:</p>
                  {semPix.length > 0 && (
                  ...
                  {pendentesLote.map((pg) => (
                    <div key={pg.id} className="bg-[#0a0a0a] border border-[#333] p-3 rounded-lg flex justify-between items-start gap-3">
                      ...
                      <div className="text-right shrink-0">
                        {pg.chavePix ? (
                          <>
                            ...
                            <button onClick={() => copiarPix(pg.chavePix!)}
                              className="flex items-center gap-1 bg-[#1a1a1a] hover:bg-[#222] border border-[#333] hover:border-green-500 rounded px-2 py-1 text-green-400 text-xs font-mono transition-colors">
                              <span className="max-w-[120px] truncate">{pg.chavePix}</span>
                              <Copy size={11} className="shrink-0" />
                            </button>
                          </>
                        ) : (
                          <span className="text-xs text-yellow-600 flex items-center gap-1"><AlertCircle size={11} /> Sem PIX</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <div className="p-5 border-t border-[#222] flex justify-between items-center">
              ...
              <button onClick={() => setModalPix(false)} className="bg-[#222] hover:bg-[#333] text-white px-6 py-2 rounded-lg text-sm font-medium transition-all duration-200 active:scale-95">Fechar</button>
            </div>
          </div>
        </div>
      )}
