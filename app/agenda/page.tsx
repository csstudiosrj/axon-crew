...
import { 
  ...
  CheckCircle2, Search, UserPlus, ChevronLeft, Edit
} from "lucide-react";
...
export default function AgendaPage() {
  ...
  // ================= LÓGICA DE ALERTAS INTELIGENTES (AGRUPADOS) =================
  const alertas = useMemo(() => {
    ...
    eventos.forEach(evento => {
      evento.slots.forEach((slot: any) => {
        ...
        if (faltam > 0) {
          ...
          faltasPorEvento[evento.id].faltas.push(`${faltam} ${slot.especialidade}(s)`);
        }

        // Agrupa alocações para achar conflitos
    ...
    // Monta os cards de alerta de falta
    Object.values(faltasPorEvento).forEach(falta => {
      novosAlertas.push({
        id: `falta-${falta.titulo}`,
        tipo: 'falta',
        titulo: `Equipe Incompleta: ${falta.titulo}`,
        desc: `Falta escalar: ${falta.faltas.join(', ')}`
      });
    });

    // Monta os cards de alerta de conflito
    Object.values(alocacoes).forEach(aloc => {
      if (aloc.eventos.length > 1) {
        novosAlertas.push({
          id: `conflito-${aloc.freela.id}`,
          tipo: 'conflito',
          titulo: `Conflito de Agenda: ${aloc.freela.nome}`,
          desc: `Escalado(a) no mesmo dia em: ${aloc.eventos.join(' e ')}`
        });
      }
    });
  ...
  const handleSalvarEvento = async (e: React.FormEvent) => {
    ...
    if (evError || !evData) {
      mostrarToast("Erro ao criar evento. Verifique a conexão.", "erro");
      return;
    }
  ...
  const handleAtualizarEvento = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      mostrarToast("Erro ao atualizar evento", "erro");
    }
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto w-full relative">
                        ...
                        {menuAberto === evento.id && (
                          <>
                            ...
                            <div className="absolute right-0 top-8 w-48 bg-[#1a1a1a] border border-[#333] rounded-lg shadow-2xl z-20 py-1 overflow-hidden animate-in fade-in">
                              <button onClick={() => { setMenuAberto(null); setModalEditarEvento(evento); }} className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-[#222] flex items-center gap-2"><Edit size={14}/> Editar Evento</button>
      ...
      {modalEditarEvento && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#121212] border border-[#222] rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-5 border-b border-[#222]">
              <h3 className="text-lg font-bold text-white flex items-center gap-2"><Edit size={18} className="text-green-500"/> Editar Evento</h3>
              <button onClick={() => setModalEditarEvento(null)} className="text-gray-500 hover:text-white"><X size={20} /></button>
            </div>
            
            <div className="p-6">
              <form id="form-editar-evento" onSubmit={handleAtualizarEvento} className="space-y-4">
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Nome do Evento / Job *</label>
            ...
            <div className="p-5 border-t border-[#222] flex justify-end gap-3 bg-[#121212]">
              <button onClick={() => setModalEditarEvento(null)} className="px-4 py-2 text-sm text-gray-400 hover:text-white">Cancelar</button>
