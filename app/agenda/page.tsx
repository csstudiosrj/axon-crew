...
export default function AgendaPage() {
  ...
  const handleAtribuirFreela = (freela: Freela) => {
    ...
    // Atualiza o modal de escala aberto para refletir a mudança imediatamente
    setModalEscala(prev => {
      if (!prev) return prev;
      return {
        ...prev,
                    ...
                    {necessidades.map((nec, index) => (
                      <div key={nec.id} className="flex items-end gap-3 bg-[#0a0a0a] p-3 rounded-lg border border-[#222]">
                        <div className="flex-1">
                          ...
                          <select value={nec.especialidade} onChange={e => setNecessidades(necessidades.map(n => n.id === nec.id ? { ...n, especialidade: e.target.value } : n))} className="w-full bg-[#121212] border border-[#333] rounded-md p-2 text-sm text-white focus:border-green-500 focus:outline-none">
